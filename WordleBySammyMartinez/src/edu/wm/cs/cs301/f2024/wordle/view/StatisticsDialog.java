...
public class StatisticsDialog extends JDialog {
	...
	public StatisticsDialog(WordleFrame view, WordleModel model) {
		super(view.getFrame(), AppStrings.STATISTICS_LABEL, true);
		this.view = view;
		this.model = model;
	...
	public StatisticsDialog(WordleFrame view, AbsurdleModel model) {
		super(view.getFrame(), AppStrings.STATISTICS_LABEL, true);
		this.view = view;
		this.model = model;
		this.exitAction = new ExitAction();
	...
	private JPanel createTitlePanel() {
		...
		JLabel label = new JLabel(AppStrings.STATISTICS_LABEL);
	...
	private JPanel createSubtitlePanel() {
		...
		JLabel label = new JLabel(AppStrings.GUESS_DISTRIBUTION_LABEL);
		label.setFont(AppFonts.getTitleFont());
		panel.add(label);
	...
	//	int percent = (wordsGuessed.size() * 1000 + 5) / (totalGamesPlayed * 10);
		int percent = (model.getTotalGamesWon()*1000+5)/(totalGamesPlayed*10);
		panel.add(createStatisticsPanel(totalGamesPlayed, AppStrings.PLAYED_LABEL, ""));
		panel.add(createStatisticsPanel(percent, AppStrings.WIN_PERCENT_LABEL, ""));
		panel.add(createStatisticsPanel(currentStreak, AppStrings.CURRENT_STREAK_LABEL, AppStrings.STREAK_LABEL));
		panel.add(createStatisticsPanel(longestStreak, AppStrings.LONGEST_STREAK_LABEL, AppStrings.STREAK_LABEL));
	...
	private JPanel createButtonPanel() {
		...
		JButton nextButton = new JButton(AppStrings.NEXT_BUTTON_TEXT);
		nextButton.addActionListener(nextAction);
		panel.add(nextButton);
		
		JButton exitButton = new JButton(AppStrings.EXIT_BUTTON_TEXT);
